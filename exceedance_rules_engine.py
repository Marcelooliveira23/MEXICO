...
_THRESHOLDS: Dict[str, Dict[str, Any]] = {
    ...
    "E145": {
        "hard_landing": {
            ...
            # G  – Inspection required (normal takeoff weight range)
            "g_low":    2.30,
    ...
    "E170": {
        "hard_landing": {
            ...
            # G  – Inspection required (high level / roll/pitch check)
            "g_high":   2.10,
    ...
    "E1": {
        "hard_landing": {
            ...
            # Adjusted to align with field-confirmed thresholds in E190/E195 reports
            # (example around 40t landing weight: ~1.97 / 2.36)
            "g_low":    1.97,
            "g_high":   2.36,
            "g_very_hard": 2.80,
            "descent_low_fpm":  600,
...
_COLUMN_ALIASES: Dict[str, List[str]] = {
    "normal_accel": [
        ...
        "vertical_acceleration_g", "normal_accel_g", "touchdown_g",
        "accvert", "fdraccelnormal1a", "fdr_accel_normal1a",
        ...
        "acceleration (normal load-factor) (g's)",
        "acceleration normal load factor", "normaccel", "acc_norm",
        "acc norm", "body_nz", "g_force_vertical", "normal g",
        "aceleracao normal", "fator de carga normal", "g vert (g)",
    ],
    "roll_rate": [
        "roll_rate", "rollrate", "roll rate", "p", "roll_rate_deg",
        "phi_dot", "roll rate (deg/s)", "roll_rate_degs",
        "ahrsbodyrollrate1a", "ahrs_body_roll_rate_1a",
        "roll angular rate", "rollrate_dps", "roll rate (deg/sec)",
        "roll speed", "taxa de roll", "taxa de rolamento",
    ],
    "pitch_rate": [
        "pitch_rate", "pitchrate", "pitch rate", "q", "pitch_rate_deg",
        "theta_dot", "pitch rate (deg/s)", "q_degs", "pitchrate_dps",
        "ahrsbodypitchrate1a", "ahrs_body_pitch_rate_1a",
        "pitch rate (deg/sec)", "pitch speed", "taxa de pitch",
    ],
    "airspeed": [
        ...
        "indicated airspeed (knots)", "airspeed (indicated)",
        "ias (knots)", "speed kts", "velocidade indicada",
    ],
    "mach": [
        "mach", "mach_number", "mach number", "mach no", "mach_no",
        "mach_speed", "mach_num", "mmo", "mach (calc)",
    ],
    "bank_angle": [
        "bank_angle", "phi", "roll_angle", "bank angle", "roll angle",
        "bank", "phi_deg", "bank_angle_deg", "bank (deg)", "roll (deg)",
        "roll attitude", "bank angle (deg)", "angulo de bank",
    ],
    "flap_position": [
    ...
    "vertical_speed": [
        ...
        "roc", "rate_of_climb", "vertical speed [fpm]", "vspeed",
        "taxa vertical", "razao de descida",
    ],
    "altitude": [
        "altitude", "alt", "height", "pressure_altitude", "baro_alt",
        "altitude_ft", "alt_ft", "altitude_msl", "altitude (ft)",
        "alt (ft)", "baro altitude", "pressure alt", "radio altitude",
        "height agl", "radio altitude (ft)", "agl", "altitude msl",
    ],
}
...
def _try_float(v: Any) -> Optional[float]:
    """Convert value to float; tolerant to units/text wrappers and decimal commas."""
    text = str(v or "").strip()
    if not text:
        return None
    try:
...
def _eval_hard_landing(
    ...
    if vs_vals:
        # Use the maximum descent magnitude because vendors differ on sign convention.
        descent = max(abs(v) for v in vs_vals)
...
def _eval_flap_overspeed(
    ...
    if flap_vals and flap_col:
        ...
        peak_ias = max(ias_vals)
        vfe_min = min(v for k, v in t.items() if k !=
                      "ref" and isinstance(v, (int, float)))
        if peak_ias > vfe_min:
            rules.append(_make_rule(
...
def _eval_gear_overspeed(
    ...
    if gear_vals and gear_col:
        ...
        for row in csv_rows:
            ...
            gear_down = (gear_v is not None and gear_v >
                         0.5) or "down" in str_gear
...
_SIGNAL_TO_EVENTS: Dict[str, List[str]] = {
    ...
    "vmo overspeed":          ["vmo_mmo"],
    "landing overspeed":      ["vmo_mmo"],
...
_EVENT_COMPANIONS: Dict[str, List[str]] = {
    "hard_landing": ["over_g"],
}


def _derive_requested_events(signals: List[str]) -> List[str]:
    requested: List[str] = []
    for signal in signals or []:
        for event_name in _SIGNAL_TO_EVENTS.get(str(signal or "").lower(), []):
            if event_name not in requested:
                requested.append(event_name)
    expanded: List[str] = list(requested)
    for event_name in requested:
        for companion in _EVENT_COMPANIONS.get(event_name, []):
            if companion not in expanded:
                expanded.append(companion)
    return expanded


def _supported_events_from_data(csv_rows: List[Dict[str, str]]) -> List[str]:
    if not csv_rows:
        return []

    supported: List[str] = []

    def _has_values(param: str) -> bool:
        values, _ = _extract_values(csv_rows, param)
        return bool(values)

    has_normal_accel = _has_values("normal_accel")
    has_vertical_speed = _has_values("vertical_speed")
    has_touchdown_g = _has_values("touchdown_g")
    has_roll_rate = _has_values("roll_rate")
    has_pitch_rate = _has_values("pitch_rate")
    has_airspeed = _has_values("airspeed")
    has_mach = _has_values("mach")
    has_flap = _has_values("flap_position")
    has_gear = _has_values("gear_position")
    has_altitude = _has_values("altitude")
    has_weight = _has_values("weight")
    has_bank_angle = _has_values("bank_angle")
    has_oat = _has_values("oat")

    if any([has_normal_accel, has_vertical_speed, has_touchdown_g, has_roll_rate, has_pitch_rate]):
        supported.append("hard_landing")
    if has_normal_accel:
        supported.append("over_g")
    if has_airspeed or has_mach:
        supported.append("vmo_mmo")
    if has_flap and has_airspeed:
        supported.append("flap_overspeed")
    if has_gear and has_airspeed:
        supported.append("gear_overspeed")
    if has_normal_accel and has_altitude:
        supported.append("turbulence_exceedance")
    if has_weight:
        supported.append("overweight_landing")
    if has_bank_angle:
        supported.append("high_bank_angle")
    if has_oat:
        supported.append("temperature_envelope")
    return supported


def _select_events_to_check(csv_rows: List[Dict[str, str]], signals: List[str]) -> List[str]:
    requested_events = _derive_requested_events(signals)
    supported_events = _supported_events_from_data(csv_rows)

    if not csv_rows:
        return requested_events or list(_DEFAULT_EVENTS)

    if requested_events:
        selected = [
            event_name for event_name in requested_events
    ...
    return supported_events or list(_DEFAULT_EVENTS)
...
def evaluate_exceedance_verdict(
    ...
    requested_events = _derive_requested_events(signals)
    supported_events = _supported_events_from_data(csv_rows)
    events_to_check: List[str] = _select_events_to_check(csv_rows, signals)
    missing_requested_events = [
        event_name for event_name in requested_events
        if csv_rows and event_name not in supported_events
    ]
    ...
    for event_name in missing_requested_events:
        ref = str((thresholds.get(event_name, {}) or {}).get("ref", "") or "")
        all_rules.append(_make_rule(
            event_name,
            "UNDETERMINED",
            "missing_supporting_columns",
            0.0,
            0.0,
            "",
            ref,
            resolved_family,
            "LOW",
            "Requested event signal found, but supporting columns were not present in the CSV.",
        ))
...
def evaluate_exceedance_suite(
    csv_rows: List[Dict[str, str]],
    tail: str = "",
    modelo: str = "",
    family: str = "",
) -> Dict[str, Any]:
    """Run a full event suite inspired by the external exceedance tool.

    Unlike the single verdict, this returns a per-event operational summary
    suitable for UI dashboards and raw FDR/ACMS files with sparse text labels.
    """
    resolved_family = _resolve_family(tail, modelo, family)
    thresholds = _THRESHOLDS.get(resolved_family, _THRESHOLDS[""])
    ...
    events_to_check = _select_events_to_check(csv_rows, [])

    for event_name in events_to_check:
        evaluator = _EVALUATORS.get(event_name)
        if evaluator is None:
            continue
        if event_name == "hard_landing":
            rules = evaluator(csv_rows, thresholds, resolved_family, [])
        else:
            rules = evaluator(csv_rows, thresholds, resolved_family)
        rules = rules or []
        ...
        yes_rules = [rule for rule in rules if rule.get("verdict") == "YES"]
        undetermined_rules = [
            rule for rule in rules if rule.get("verdict") == "UNDETERMINED"]
        strongest_rule = None
        if yes_rules:
            strongest_rule = max(
                yes_rules,
                key=lambda rule: _SEVERITY_MAP.get(
                    str(rule.get("severity", "NONE") or "NONE"), 0),
            )
            status = "ALERT"
            summary = strongest_rule.get(
                "note") or f"Threshold exceeded in {event_name}."
            severity = str(strongest_rule.get("severity", "HIGH") or "HIGH")
            critical_findings.append(
                f"{_EVENT_DISPLAY_NAMES.get(event_name, event_name)}: {summary}"
            )
        elif undetermined_rules:
            strongest_rule = undetermined_rules[0]
            status = "REVIEW"
            summary = strongest_rule.get(
                "note") or f"Insufficient parameter coverage for {event_name}."
            severity = str(strongest_rule.get(
            ...
            status = "NORMAL"
            summary = f"No exceedance detected for {_EVENT_DISPLAY_NAMES.get(event_name, event_name)}."
            severity = "NONE"

        event_summaries.append({
            "event": event_name,
            "label": _EVENT_DISPLAY_NAMES.get(event_name, event_name),
            "status": status,
            "severity": severity,
            "trigger_count": len(yes_rules),
            "review_count": len(undetermined_rules),
            "summary": summary,
            "top_rule": strongest_rule,
        })

    parameter_peaks: Dict[str, float] = {}
    evaluated_parameters: List[Dict[str, Any]] = []
    for param in _COLUMN_ALIASES:
        values, resolved_col = _extract_values(csv_rows, param)
        if not values:
            continue
        if param in ("normal_accel", "bank_angle", "roll_rate"):
    ...
    normal_events = sum(
        1 for item in event_summaries if item["status"] == "NORMAL")
    review_events = sum(
        1 for item in event_summaries if item["status"] == "REVIEW")
    alert_events = sum(
        1 for item in event_summaries if item["status"] == "ALERT")
    conformance_score = round(
        (normal_events / max(1, total_events)) * 100.0, 1)
    overall_status = "ALERT" if alert_events else "REVIEW" if review_events else "NORMAL"
    ...
    return {
        ...
        "warnings": warnings,
        "parameter_peaks": parameter_peaks,
